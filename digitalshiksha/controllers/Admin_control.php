...
class Admin_control extends MS_Controller {
    ...
    public function __construct()
    {
        ...
        // Allow guest access to AJAX category methods
        $allowed_guest_methods = array('get_subcategories_ajax', 'get_sub_subcategories_ajax');
        $current_method = $this->router->method;
        
        if (!in_array($current_method, $allowed_guest_methods)) {
            if (!$this->session->userdata('log')) {
                $this->session->set_userdata('back_url', current_url());
                // Dashboard routes should land on the dashboard-style login entrypoint
    ...
    {
        // Ensure exam_type is set correctly from route
        if (empty($exam_type)) {
            $exam_type = $this->uri->segment(2);
            // Handle both underscore and hyphen formats
            if ($exam_type == 'mock-test') {
                $exam_type = 'mock_test';
            } elseif ($exam_type == 'live-mock-test') {
        ...
        $config['per_page'] = 10;
        $config["base_url"] = base_url("mocks/{$exam_type}");
        ...
        if ($this->session->userdata('user_role_id') == 1 || $this->session->userdata('user_role_id') == 3) {
            ...
            $config["total_rows"] = count($this->exam_model->get_all_mocks($exam_type));
            $this->pagination->initialize($config);
            $data["links"] = $this->pagination->create_links();
            $data['mocks'] = $this->exam_model->get_all_mocks($exam_type, $config['per_page'], $offset);
            $data['content'] = $this->load->view('content/view_all_mocks', $data, TRUE);
        } else {
            $config["total_rows"] = $this->admin_model->count_user_mocks($exam_type, $userId);
            $this->pagination->initialize($config);
            $data["links"] = $this->pagination->create_links();
            $data['mocks'] = $this->admin_model->get_user_mocks($exam_type, $userId, $config['per_page'], $offset);
            $data['exam_type'] = $exam_type;
    ...
    public function export_exam_questions($id)
    {
        ...
        foreach ($data as $data_row) {
            ...
            {
                $imagePath = FCPATH.'uploads/question-media/'.$data_row['media_link'];
    ...
    public function view_my_mock_detail($id, $message = '')
    {
        ...
        // If get_mock_by_id fails (due to JOIN issues), fall back to MockDetails
        if (empty($data['mock_title'])) {
            // Use basic MockDetails data if JOIN fails
            $data['mock_title'] = $MockDetails;
        }
        
        // Check user permissions - allow role 1 (super admin), 3 (moderator), 4 (teacher), or owner
        $user_role_id = isset($this->session->userdata['user_role_id']) ? $this->session->userdata['user_role_id'] : 0;
        $user_id = isset($this->session->userdata['user_id']) ? $this->session->userdata['user_id'] : 0;
        $is_admin_or_moderator = ($user_role_id == 1 || $user_role_id == 3 || $user_role_id == 4);
        $is_owner = (isset($MockDetails->user_id) && $MockDetails->user_id == $user_id);
        
        if ($is_admin_or_moderator || $is_owner) {
...
//        }
        ...
        $data['sidebar'] = $this->load->view('sidebar/admin_sidebar', $data, TRUE);
        $data['message'] = $message;
        $categories = $this->exam_model->get_categories_with_author();
        
        // Calculate total exam count for each category including sub-categories and sub-sub-categories
        $mock_count = array();
        foreach ($categories as $category) {
            $total_count = 0;
            
            // Count exams directly under this category
            $direct_count = $this->db->where('category_id', $category->category_id)
                ->where('active', 1)
                ->from('exam_title')
                ->count_all_results();
            ...
            $sub_categories = $this->exam_model->get_subcategories_by_catid($category->category_id);
            $sub_category_ids = array();
            foreach ($sub_categories as $sub_cat) {
                $sub_category_ids[] = $sub_cat->id;
                
                // Count exams in this sub-category
                $sub_count = $this->db->where('sub_category_id', $sub_cat->id)
                    ->where('active', 1)
                    ->from('exam_title')
                    ->count_all_results();
                $total_count += $sub_count;
                
                // Get all sub-sub-categories for this sub-category
                $sub_sub_categories = $this->exam_model->get_subsubcategories_by_catid($sub_cat->id);
                foreach ($sub_sub_categories as $sub_sub_cat) {
                    // Count exams in this sub-sub-category
                    $sub_sub_count = $this->db->where('sub_sub_category_id', $sub_sub_cat->id)
                        ->where('active', 1)
                        ->from('exam_title')
                        ->count_all_results();
                    $total_count += $sub_sub_count;
                }
            }
            
            $mock_count[$category->category_id] = $total_count;
        }
        
        $data['categories'] = $categories;
        $data['mock_count'] = $mock_count;
    ...
    { 
        ...
        // Calculate exam count for each subcategory: exams under category OR under subcategory
        $mock_count = array();
        foreach ($data['sub_categories'] as $sub_category) {
            $total_count = $this->db->group_start()
                ->where('category_id', $sub_category->cat_id)
                ->or_where('sub_category_id', $sub_category->id)
                ->group_end()
                ->where('active', 1)
                ->from('exam_title')
                ->count_all_results();
            $mock_count[$sub_category->id] = $total_count;
        }
        $data['mock_count'] = $mock_count;
    ...
    { 
        ...
        // Calculate exam count for each category and sort by exam count
        $category_exam_counts = array();
        foreach ($categories as $category) {
            $total_count = $this->db->group_start()
            ...
            // Also count exams in sub-categories
            $sub_categories = $this->exam_model->get_subcategories_by_catid($category->category_id);
            foreach ($sub_categories as $sub_cat) {
                $sub_count = $this->db->where('sub_category_id', $sub_cat->id)
                    ->where('active', 1)
                ...
                // Count exams in sub-sub-categories
                $sub_sub_categories = $this->exam_model->get_subsubcategories_by_catid($sub_cat->id);
                foreach ($sub_sub_categories as $sub_sub_cat) {
                    $sub_sub_count = $this->db->where('sub_sub_category_id', $sub_sub_cat->id)
                        ->where('active', 1)
            ...
            $category->exam_count = $total_count;
            $category_exam_counts[$category->category_id] = $total_count;
        }
        
        // Sort categories by exam count (descending)
        ...
        if($_GET['sub_category'] && $_GET['category'])
        {
            ...
            // Calculate exam count for each sub-category and sort by exam count
            foreach ($sub_categories as $sub_cat) {
                $total_count = 0;
                
                // Count exams directly under this sub-category
                $sub_count = $this->db->where('sub_category_id', $sub_cat->id)
                    ->where('active', 1)
                    ->from('exam_title')
                    ->count_all_results();
                ...
                // Also count exams in sub-sub-categories under this sub-category
                $sub_sub_cats = $this->exam_model->get_subsubcategories_by_catid($sub_cat->id);
                ...
                $sub_cat->exam_count = $total_count;
            }
            
            // Sort sub-categories by exam count (descending)
            ...
            // Calculate exam count for each sub-subcategory
            $mock_count = array();
            foreach ($data['sub_sub_categories'] as $sub_sub_category) {
                $total_count = $this->db->where('sub_sub_category_id', $sub_sub_category->id)
                    ->where('active', 1)
                    ->from('exam_title')
                    ->count_all_results();
                $mock_count[$sub_sub_category->id] = $total_count;
            }
            $data['mock_count'] = $mock_count;
        } else {
    ...
    {  
        ...
        // Get exam type from URI segment if type parameter is empty
        if (empty($type)) {
            $type = $this->uri->segment(3);
        }
        
        // Set class based on exam type - 22 for mock_test, 102 for live_mock_test
        if ($type == 'mock_test') {
            $data['class'] = 22;
        } elseif ($type == 'live_mock_test') {
            $data['class'] = 102;
        } else {
            $data['class'] = 97; // default
        }
        ...
        $data['exam_type'] = $type ? $type : $this->uri->segment(3);
...
//                $config['max_height'] = '768';
                ...
                if($this->input->post('exam_type', TRUE) == 'live_mock_test') { 
                   return redirect(base_url('mock_detail/'.$title_id));
                    ...
                    $this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissable">'
                        ...
                        . '</div>');
                    return redirect(base_url('admin_control/add_more_question/'.$title_id));
                }
    ...
    public function resizeImage($filename)
   {
    ...
    public function create_question($message = '')
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            return redirect(base_url('admin_control/add_more_question/'.$this->input->post('ques_id')));
        } else{
            ...
            if (isset($_FILES['media']['name']) && !empty($_FILES['media']['name'])) {
                $config['upload_path'] = './uploads/question-media/'.$this->input->post('media_type').'/';
            ...
            if ($this->admin_model->add_question($file_name, $file_type))
            {
                if ($this->input->post('done')) {                    
                    ...
                    return redirect(base_url('admin_control/add_more_question/'.$exam_id.'/'.$question_no));
                }
            } else {
    ...
    public function add_more_question($id = '', $question_no = '')
    {
        if (!is_numeric($id)) {
            show_404();
        }
...
//        } else {
            ...
            if (empty($question_no) || !is_numeric($question_no)) {
                $question_no = $this->exam_model->question_count_by_id($id) + 1;
            }
            // Don't pass message as parameter, let the view read from flashdata directly
            $this->question_form('', $title_id, $mock_title, $question_no);
        //}
    }
