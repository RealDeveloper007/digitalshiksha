...
class System_control extends MS_Controller 
{
    ...
    public function view_settings($message = '')
    {
        ...
        if($_POST){
            // Check if this is a color-only update (from Theme tab)
            $is_color_update = $this->input->post('update_type') === 'colors_only' || ($this->input->post('primary_color') && !$this->input->post('brand_name'));
            
            if ($is_color_update) {
                // Color-only update - no validation needed
                $info = array();
                $info['primary_color'] = $this->input->post('primary_color', TRUE) ?: '#2c3e50';
                $info['secondary_color'] = $this->input->post('secondary_color', TRUE) ?: '#34495e';
                $info['accent_color'] = $this->input->post('accent_color', TRUE) ?: '#FFD700';
                $info['accent_color_alt'] = $this->input->post('accent_color_alt', TRUE) ?: '#F1B900';
                $info['text_color'] = $this->input->post('text_color', TRUE) ?: '#333333';
                $info['text_color_light'] = $this->input->post('text_color_light', TRUE) ?: '#666666';
                $info['background_color'] = $this->input->post('background_color', TRUE) ?: '#ffffff';
                $info['background_dark'] = $this->input->post('background_dark', TRUE) ?: '#1a1a1a';
                ...
                if ($this->db->affected_rows() >= 0) {
                    ...
                    $message = '<div class="alert alert-success alert-dismissable">'
                            ...
                            . 'Color settings updated successfully!'
                            . '</div>';
                }
            } else {
                // Basic info update - validate required fields
                ...
                if ($this->form_validation->run() != FALSE) {
                    ...
                    // Color Theme Settings (if provided)
