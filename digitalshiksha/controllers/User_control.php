...
class User_control extends MS_Controller
{
    ...
    public function edit_user()
    {
        ...
        if ($this->form_validation->run() == FALSE) 
        {
            ...
            // Category assignment is role-based:
            // Teacher (4): save selected categories
            // Student (5): keep optional and store NULL
            if ((int)$user_details->user_role_id === 4) {
                $assign_categories = $this->input->post('assign_categories');
                if (is_array($assign_categories) && !empty($assign_categories)) {
                    $assign_categories = array_filter(array_map('trim', $assign_categories), static function ($value) {
                        return $value !== '';
                    });
                    $info['assign_categories'] = !empty($assign_categories) ? implode(',', $assign_categories) : null;
                } else {
                    $info['assign_categories'] = null;
                }
            } else {
              ...
              $CheckEmailExist = $this->login_model->check_user_mail_exist($this->input->post('user_email'),$user_id);           

             $CheckPhoneExist = $this->login_model->check_user_phone_exist($this->input->post('user_phone'),$user_id);
    ...
    public function view_user_detail($id)
    {
        ...
        $data = array();
        $data['class'] = 11; // class control value left digit for main menu right digit for submenu
        $data['header'] = $this->load->view('header/admin_head', $data, TRUE);
        $data['top_navi'] = $this->load->view('header/admin_top_navigation', $data, TRUE);
        $data['sidebar'] = $this->load->view('sidebar/admin_sidebar', $data, TRUE);
        $data['user_details'] = $this->admin_model->get_user_details($id);
        ...
        $data['content'] = $this->load->view('content/view_user_detail', $data, TRUE);
        $data['footer'] = $this->load->view('footer/admin_footer', $data, TRUE);
        $this->load->view('dashboard', $data);
    }
