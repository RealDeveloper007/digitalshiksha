...
class Login_control extends MS_Controller
{
    ...
    public function index($message = '')
    {
        if ($_POST) 
        {
            // Default to student (5) if user_role not provided (backward compatibility)
            ...
            $this->form_validation->set_rules('user_role', 'User Type', 'in_list[4,5]');
        ...
        $data['top_navi'] = $this->load->view('header/top_navigation', $data, TRUE); // Show header navigation
        
        // Read flashdata once and store it
        // CodeIgniter will automatically clear flashdata after this request
        $flash_message = $this->session->flashdata('message');
        
        // Store the message - prioritize flashdata if available, otherwise use passed $message
        $data['message'] = $flash_message ? $flash_message : $message;
        
        // Clear flashdata immediately after reading to prevent it from showing on other pages
        // This ensures the message only shows once on the login page
        if ($flash_message) {
            // Unset the flashdata from session to prevent persistence
            $this->session->unset_userdata('message');
            // Also clear the flashdata marker in __ci_vars
            $ci_vars = $this->session->userdata('__ci_vars');
            if (is_array($ci_vars) && isset($ci_vars['message'])) {
                unset($ci_vars['message']);
        ...
        $data['footer'] = $this->load->view('footer/footer', $data, TRUE); // Show footer
    ...
    private function SendOTP($mobile)
    {
        ...
        // }
            ...
            // Account details
            $apiKey = 'OWFjYjQxOTk5NDA0NDAwYzNhZDQ4NjdkOGVmYTdmMmY=';  // Replace with your actual API key
            
            // Message details
            $numbers = array('91'.$mobile);  // Add valid numbers here
            $sender = urlencode('DIGISD');  // Sender ID
            $message = rawurlencode("Dear Customer, Your OTP is " . $OTP . ". Kindly keep is secret for login. www.digitalshikshadarpan.com");  // Your message
            
            // Convert numbers array into a comma-separated string
            ...
            if (curl_errno($ch)) {
                $error = curl_error($ch);
                curl_close($ch);
                log_message('error', 'SMS cURL Error: ' . $error);
                // Still return OTP for testing purposes - remove this in production if SMS is critical
                return $OTP;
            }
            ...
            // Check for errors
            if($response === false) {
                log_message('error', 'SMS API returned false');
                // Still return OTP for testing - remove in production if SMS is critical
                return $OTP;
            } else {
                // Process the response (you can print it for debugging)
                // echo $response;
                return $OTP;
            }
    }
    ...
    {
        // Set JSON response header and disable output buffering
        ...
        try {
            ...
            if (!$this->input->post('type') || !$this->input->post('type_value')) {
                $response = array('status' => false, 'type' => 'missing_data', 'message' => 'Missing required fields: type and type_value');
            ...
            if ($this->form_validation->run() === FALSE) {
                $response = array('status' => false, 'type' => 'validation_error', 'message' => validation_errors());
                echo json_encode($response);
                exit();
            }

            // Check if OTP verification (existing OTP being verified)
            // The saveOtps model method handles OTP verification when token and otp are in POST data
            if ($this->input->post('otp') && $this->input->post('token')) {
                // Pass any value as OTP parameter - model will check POST data for actual verification
                // The model first checks if token exists, if yes it verifies the OTP from POST data
                $VerifyOtpData = $this->login_model->saveOtps($this->input->post('otp'));
                
                if ($VerifyOtpData && isset($VerifyOtpData['status']) && $VerifyOtpData['status']) {
                    $response = array('status' => true, 'type' => 'otp_checked');
                    echo json_encode($response);
                    exit();
                } else {
                    $response = array('status' => false, 'type' => 'otp_not_checked', 'message' => 'Invalid OTP. Please try again.');
                    echo json_encode($response);
                    exit();
                }
            }
            ...
            if ($GetOTP && $GetOTP !== false) {
                // The saveOtps method expects POST data to be available for type and type_value
                $SaveOtpData = $this->login_model->saveOtps($GetOTP);

                if ($SaveOtpData && isset($SaveOtpData['status']) && $SaveOtpData['status']) {
                    $response = array(
                        'status' => true, 
                        'type' => isset($SaveOtpData['type']) ? $SaveOtpData['type'] : 'otp_sent', 
                        'data' => isset($SaveOtpData['insert_id']) ? md5($SaveOtpData['insert_id']) : ''
                    );
                    echo json_encode($response);
                    exit();
                } else {
                    $response = array(
                        'status' => false, 
                        'type' => isset($SaveOtpData['type']) ? $SaveOtpData['type'] : 'otp_save_failed',
                ...
                $response = array('status' => false, 'type' => 'otp_not_sent', 'message' => 'Failed to send OTP. Please try again.');
                echo json_encode($response);
                exit();
            }
    ...
    private function SendOtpOnEmail($email)
    {
        ...
        // Check if session data exists, use defaults if not
        $support_email = isset($this->session->userdata['support_email']) ? $this->session->userdata['support_email'] : 'noreply@digitalshikshadarpan.com';
        $brand_name = isset($this->session->userdata['brand_name']) ? $this->session->userdata['brand_name'] : 'Digital Shiksha';
        $support_phone = isset($this->session->userdata['support_phone']) ? $this->session->userdata['support_phone'] : '';
        $address = isset($this->session->userdata['address']) ? $this->session->userdata['address'] : '';
        
        $to = $email;
        $suject = 'OTP Sent on ' . $Date . ' | ' . $brand_name;
        $Data['name'] = $this->input->post('user_name');
        $Data['brand'] = $brand_name;
        $Data['brand_phone'] = $support_phone;
        ...
        try {
            $this->email->initialize($config);
            $this->load->library('email', $config);
            $this->email->clear(TRUE); // Clear previous email data
            $this->email->set_newline("\r\n");
            $this->email->from($support_email, $brand_name);
            $this->email->to($to);
            $this->email->subject($suject);
            ...
            if ($this->email->send()) {
                ...
                // For testing, return OTP even if email fails - remove in production
                return $Data['otp'];
            }
        } catch (Exception $e) {
