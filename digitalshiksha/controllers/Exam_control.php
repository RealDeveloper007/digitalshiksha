...
class Exam_control extends MS_Controller
{
    ...
    {
        // Allow guest access to view mock test listing page
        ...
        // Get total count first (without pagination limit) - for all filtered results
        if (isset($_GET['main_category']) && isset($_GET['sub_category']) && isset($_GET['sub_sub_category']) && $_GET['main_category'] && $_GET['sub_category'] && $_GET['sub_sub_category']) {
            $all_mocks = $this->exam_model->get_mocks_by_category($_GET['main_category'], $_GET['sub_category'], $_GET['sub_sub_category']);
        } else {
            $all_mocks = $this->exam_model->get_all_mocks('mock_test');
        }
        
        // Filter out inactive or non-public exams for count
        $total_exams = 0;
        foreach ($all_mocks as $mock) {
            if (isset($mock->exam_active) && $mock->exam_active == 1 && isset($mock->public) && $mock->public == 1) {
                $total_exams++;
            }
        }
        ...
        if (isset($_GET['page']) && $_GET['page']) {
            ...
            $page = 1; // First page when use_page_numbers is TRUE
        }

        $config['use_page_numbers'] = TRUE;
        $config['page_query_string'] = TRUE;
        $config['query_string_segment'] = 'page';
        $config['per_page']         = 39; // Multiple of 3 to ensure 3 exams per row (13 rows × 3 = 39)
        $config["base_url"]         = base_url() . "mock-test";
        $config["total_rows"]       = $total_exams; // Use filtered count
        ...
        $get_params = $_GET;
        unset($get_params['page']); // Remove page from params to avoid duplication
        ...
        if (isset($_GET['main_category']) && isset($_GET['sub_category']) && isset($_GET['sub_sub_category']) && $_GET['main_category'] && $_GET['sub_category'] && $_GET['sub_sub_category']) {
            $data['mocks'] = $this->exam_model->get_mocks_by_category($_GET['main_category'], $_GET['sub_category'], $_GET['sub_sub_category'], $config["per_page"], ($page > 0 ? ($page - 1) * $config["per_page"] : 0));
            $data['category_name'] = $this->db->get_where('sub_categories', array('id' => $_GET['sub_category']))->row()->sub_cat_name;

        } else if (isset($_GET['exam_name']) && $_GET['exam_name']) {
            // Search by exam name or code - handled in model
            $data['mocks'] = $this->exam_model->get_all_mocks('mock_test', $config["per_page"], ($page > 0 ? ($page - 1) * $config["per_page"] : 0));
        } else {
    ...
    {
        // Allow guest access to view study material listing page
        $this->load->model('system_model');
        $this->system_model->set_system_info_to_session();
        ...
        // Get all syllabus by default, or filter by category if provided
        if (isset($_GET['main_category']) && isset($_GET['sub_category']) && isset($_GET['sub_sub_category']) 
            && !empty($_GET['main_category']) && !empty($_GET['sub_category']) && !empty($_GET['sub_sub_category'])) {
            $data['syllabus'] = $this->exam_model->get_syllabus_by_category($_GET['main_category'], $_GET['sub_category'], $_GET['sub_sub_category']);

            $category_row = $this->db->get_where('sub_categories', array('id' => $_GET['sub_category']))->row();
            if ($category_row) {
                $data['category_name'] = $category_row->sub_cat_name;
            } else {
                $data['category_name'] = '';
            }
        } else {
            // Show all syllabus by default
            $data['syllabus'] = $this->exam_model->get_all_syllabus();
            $data['category_name'] = '';
        }
        
        // Count total materials
        $data['total_materials'] = isset($data['syllabus']) ? count($data['syllabus']) : 0;
        $data['categories'] = $this->exam_model->get_categories();
        $data['top_navi'] = $this->load->view('header/top_navigation', $data, TRUE);
        $data['user_role'] = $this->admin_model->get_user_role();
        $data['message'] = $message;
    ...
    {
        // Require login to view individual exam details
        if (!$this->session->userdata('log')) {
            $this->session->set_userdata('back_url', current_url());
            $message = '<div class="alert alert-danger alert-dismissable">'
                . '<button type="button" class="close" data-dismiss="alert" aria-hidden="TRUE">&times;</button>'
                . 'Please login to view exam details!</div>';
    ...
    public function start_exam($id = '', $message = '')
    {
        ...
        $data['footer'] = ''; // Removed footer for start exam page
        $this->load->view('home', $data);
    ...
    public function download_result_pdf($id = '')
    {
        if (!$this->session->userdata('log')) {
            $this->session->set_userdata('back_url', current_url());
            redirect(base_url('login_control'));
        ...
        $result = $this->exam_model->view_result_detail($id);
        if (empty($result)) {
            show_404();
        }

        // Student can only access own result PDF
        if ((int)$this->session->userdata('user_role_id') === 5 && (int)$result->participant_id !== (int)$this->session->userdata('user_id')) {
            exit('<h2>You are not Authorised person to do this!</h2>');
        }

        $all_questions = $this->exam_model->get_mock_detail($result->exam_id);
        $right_answers = 0;
        $attempted = 0;
        $student_answers = json_decode($result->result_json);

        if (!empty($all_questions) && !empty($student_answers)) {
            foreach ($all_questions as $question) {
                $check = $this->StudentQuestionAnswer($student_answers, $question->ques_id, $result->exam_id);
                if ($check !== 'not_attempted') {
                    $attempted++;
                }
                if ($check === 'right') {
                    $right_answers++;
        ...
        $data['right_answer_count'] = $right_answers;
        ...
        $student_name_slug = strtolower(trim(preg_replace('/[^A-Za-z0-9]+/', '-', (string)$result->user_name), '-'));
        $exam_name_slug = strtolower(trim(preg_replace('/[^A-Za-z0-9]+/', '-', (string)$result->title_name), '-'));

        if ($student_name_slug === '') {
            $student_name_slug = 'student';
        }
        if ($exam_name_slug === '') {
            $exam_name_slug = 'exam';
        }
    ...
    private function build_exam_title_image($title = '')
    {
        ...
        if ($title === '' || !extension_loaded('imagick')) {
            return '';
        }

        $font_path = FCPATH . 'assets/fonts/Lohit-Devanagari.ttf';
        ...
        try {
            ...
            $draw->setFillColor(new ImagickPixel('#1f2933'));
            $draw->setTextAntialias(true);

            $probe = new Imagick();
            $metrics = $probe->queryFontMetrics($draw, $title);
            $text_width = isset($metrics['textWidth']) ? (int)ceil($metrics['textWidth']) : 400;
            $text_height = isset($metrics['textHeight']) ? (int)ceil($metrics['textHeight']) : 30;

            $canvas_width = max(420, $text_width + 28);
            $canvas_height = max(36, $text_height + 14);

            $image = new Imagick();
            $image->newImage($canvas_width, $canvas_height, new ImagickPixel('transparent'));
            $image->setImageFormat('png');

            $baseline_y = max(24, (int)round(($canvas_height + $text_height) / 2) - 4);
            $image->annotateImage($draw, 14, $baseline_y, 0, $title);

            $png_blob = $image->getImageBlob();
            $image->clear();
            $image->destroy();
            $probe->clear();
            $probe->destroy();

            return 'data:image/png;base64,' . base64_encode($png_blob);
        } catch (Exception $e) {
            return '';
        }
    }
