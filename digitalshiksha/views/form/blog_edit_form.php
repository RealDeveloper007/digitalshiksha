...
<div class="blog-form-wrapper">
    ...
    <div class="form-body">
        ...
        <?= form_open(base_url('blog/update/'.$blog->blog_id), 'method="post" role="form" class="form-horizontal" enctype="multipart/form-data" id="blog-edit-form"'); ?>
        
        <?php echo form_hidden('existing_media_type', $blog->blog_media_type); ?>
        <?php echo form_hidden('existing_attachment', $blog->blog_attachment); ?>
        <?php echo form_hidden('media_type', $blog->blog_media_type, 'id="selected_media_type"'); ?>
        
        <div class="form-group">
            <label for="blog_title">
                <i class="fa fa-header"></i> Title <span class="required">*</span>
            </label>
            <?php echo form_input('blog_title', $blog->blog_title, 'placeholder="Post title" id="blog_title" class="form-control" required="required"') ?>
        </div>
            ...
            <?= form_input('blog_slug', $blog->blog_slug, 'placeholder="Blog Slug" id="blog_slug" class="form-control" required="required"') ?>
                </div>
            ...
            <a href="#" class="btn btn-default" id='upload-media-file' style="padding: 8px 16px;">
                <i class="fa fa-upload"></i> Change Media
            </a>
                </div>
            ...
            <div class="form-group">
            <label>
                <i class="fa fa-image"></i> Current Media
            </label>
            <div class="existing-media">
                <?php if($blog->blog_media_type == 'image') { ?>
                    <img src="<?= base_url('uploads/blog_files/').$blog->blog_attachment; ?>" alt="Current image">
                    <p style="margin-top: 8px; color: #666; font-size: 13px;">Current image file</p>
                <?php } else if($blog->blog_media_type == 'file') { ?>
                    <a href="<?= base_url('uploads/blog_files/').$blog->blog_attachment; ?>" target="_blank">
                        <i class="fa fa-file"></i> View Current File
                    </a>
                <?php } else { ?>
                    <div class="video-url">
                        <strong>YouTube Video URL:</strong><br>
                        <?= htmlspecialchars($blog->blog_attachment); ?>
                    </div>
                <?php } ?>
				            </div>
				</div>
        ...
        <?php $this->load->view('plugin_scripts/bootstrap-wysihtml5'); ?>
                            <script>
                                var blogBodyEditor = CKEDITOR.replace('blog_body', {
                                    versionCheck : false,
                                    allowedContent: true,
                                    extraAllowedContent: '*(*);*{*};*[*]',
                                ...
                                blogBodyEditor.on('instanceReady', function() {
                                    blogBodyEditor.on('change', function() { this.updateElement(); });
                                    blogBodyEditor.on('blur', function() { this.updateElement(); });
                                    blogBodyEditor.on('paste', function() {
                                        var editor = this;
...
<script>
var currentMediaType = <?= json_encode($blog->blog_media_type); ?>;
var currentVideoValue = <?= json_encode(($blog->blog_media_type === 'video') ? $blog->blog_attachment : ''); ?>;

$('#upload-media-file').click(function(e){
    e.preventDefault();
    var checkedFile = (currentMediaType === 'file') ? ' checked="checked"' : '';
    var checkedImage = (currentMediaType === 'image') ? ' checked="checked"' : '';
    var checkedVideo = (currentMediaType === 'video') ? ' checked="checked"' : '';
    ...
    var type = '<div class="form-group">'
                ...
                +'<div class="media-type-options">'
                        +'<div class="media-type-option">'
                            +'<input type="radio" value="file" id="media_type_file" name="media_type_choice" required="required" onclick="add_media_field(this.value)"' + checkedFile + '>'
                            +'<label for="media_type_file">File</label>'
                        +'</div>'
                        +'<div class="media-type-option">'
                            +'<input type="radio" value="image" id="media_type_image" name="media_type_choice" required="required" onclick="add_media_field(this.value)"' + checkedImage + '>'
                            +'<label for="media_type_image">Image</label>'
                        +'</div>'
                        +'<div class="media-type-option">'
                            +'<input type="radio" value="video" id="media_type_video" name="media_type_choice" required="required" onclick="add_media_field(this.value)"' + checkedVideo + '>'
    ...
    add_media_field(currentMediaType);
});

function add_media_field(val) {
    $('#selected_media_type').val(val);
    ...
    if(val == 'video'){
        field += '<textarea id="media" name="media" class="form-control" placeholder="YouTube Embed Code" rows="3">'+ currentVideoValue +'</textarea>';
    }
...
$('#blog-edit-form').on('submit', function() {
    if (typeof blogBodyEditor !== 'undefined' && blogBodyEditor) {
        blogBodyEditor.updateElement();
    } else if (typeof CKEDITOR !== 'undefined') {
        CKEDITOR.instances.blog_body && CKEDITOR.instances.blog_body.updateElement();
    }
});
</script>
