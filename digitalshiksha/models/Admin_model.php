...
class Admin_model extends CI_Model
{
   ...
   public function get_mock_detail($id)
     ...
     * @param       id      (int)   
     */
    public function get_user_mocks($examId, $id, $limit = null, $start = null)
    {
        if (!is_numeric($id)) {
            return FALSE;
        }
        $exam_name = $this->input->get('name') ? trim($this->input->get('name')) : null;
        ...
        if (!empty($exam_name)) {
            $this->db->like('exam_title.title_name', $exam_name);
        }
    ...
    public function count_user_mocks($examId, $id)
    {
        if (!is_numeric($id)) {
            return 0;
        }
...
public function create_subcategory($sub_cat_name)
    ...
    public function get_subcategories_by_cat_id($id)
    {
        ...
        $this->db->where('sub_cat_active', 1); // Only active sub categories
      ...
      public function get_sub_subcategories_by_cat_id($id)
    {
        ...
        $this->db->where('status', 1); // Only active sub sub categories
