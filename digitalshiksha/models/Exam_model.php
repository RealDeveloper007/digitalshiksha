...
class Exam_model extends CI_Model
{
    ...
    public function get_all_mocks($examId = null, $limit = null, $start = null)
    {
        ...
        $exam_name = $this->input->get('exam_name') ? $this->input->get('exam_name') : ($this->input->get('name') ? $this->input->get('name') : null);
        ...
        // Apply exam name or code filter if provided
        if ($exam_name) {
            $search_term = trim($exam_name);
            $is_code = false;
            $exam_id = null;
            
            // Try to extract exam ID from code format (MT001, MT01, MT1, etc.)
            if (preg_match('/^MT0*(\d+)$/i', $search_term, $matches)) {
                $exam_id = (int)$matches[1];
                $is_code = true;
            } elseif (preg_match('/^MT(\d+)$/i', $search_term, $matches)) {
                $exam_id = (int)$matches[1];
                $is_code = true;
            } elseif (is_numeric($search_term)) {
                $exam_id = (int)$search_term;
                $is_code = true;
            }
            
            if ($is_code && $exam_id) {
                // Search by exam ID (code)
                $this->db->where('exam_title.title_id', $exam_id);
            } else {
                // Search by exam name
                $this->db->like('exam_title.title_name', $search_term);
            }
        }
        ...
        // Group by exam
        $this->db->group_by('exam_title.title_id');
        ...
        switch ($sort_by) {
            case 'new_exam':
                ...
                break;
            case 'old_exam':
                $this->db->order_by("exam_created", "asc")
                         ->order_by("attempt_count", "desc");
                break;
            case 'most_attempted':
            default:
    ...
    {
        // Get categories sorted by exam count (most exams first)
        $this->db->select('categories.*, COUNT(exam_title.title_id) as exam_count');
        $this->db->from('categories');
        $this->db->join('exam_title', 'exam_title.category_id = categories.category_id AND exam_title.batch_id = 0 AND exam_title.active = 1 AND exam_title.public = 1', 'left');
        $this->db->group_by('categories.category_id');
        $this->db->order_by('exam_count', 'DESC'); // Most exams first
        $this->db->order_by('category_name', 'ASC'); // Then alphabetically
        $result = $this->db->get()->result();
        return $result;
    }
    ...
    {
        // Get filter parameters
        $exam_name = $this->input->get('exam_name') ? $this->input->get('exam_name') : null;
        $sort_by = $this->input->get('sort_by') ? $this->input->get('sort_by') : 'most_attempted';
    ...
    {
        $result = $this->db->select('exam_title.*, sub_categories.*, categories.*, users.*, exam_title.active AS exam_active, COUNT(result.result_id) as attempt_count')
            ->from('exam_title')
