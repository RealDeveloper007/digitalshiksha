...
class System_model extends CI_Model
{
    ...
    public function set_system_info_to_session()
    {
        ...
        if ($result) {
            ...
            $this->session->set_userdata('linkedin_url', isset($result->linkedin_url) ? $result->linkedin_url : '');
            $this->session->set_userdata('support_email', $result->support_email);
            $this->session->set_userdata('support_phone', $result->support_phone);
            ...
            // Set color theme settings with defaults
            $this->session->set_userdata('primary_color', isset($result->primary_color) && !empty($result->primary_color) ? $result->primary_color : '#2c3e50');
            $this->session->set_userdata('secondary_color', isset($result->secondary_color) && !empty($result->secondary_color) ? $result->secondary_color : '#34495e');
            $this->session->set_userdata('accent_color', isset($result->accent_color) && !empty($result->accent_color) ? $result->accent_color : '#FFD700');
            $this->session->set_userdata('accent_color_alt', isset($result->accent_color_alt) && !empty($result->accent_color_alt) ? $result->accent_color_alt : '#F1B900');
            $this->session->set_userdata('text_color', isset($result->text_color) && !empty($result->text_color) ? $result->text_color : '#333333');
            $this->session->set_userdata('text_color_light', isset($result->text_color_light) && !empty($result->text_color_light) ? $result->text_color_light : '#666666');
            $this->session->set_userdata('background_color', isset($result->background_color) && !empty($result->background_color) ? $result->background_color : '#ffffff');
            $this->session->set_userdata('background_dark', isset($result->background_dark) && !empty($result->background_dark) ? $result->background_dark : '#1a1a1a');
            ...
            // Parse YouTube URL for video ID if needed
            if (!empty($result->you_tube_url)) {
                parse_str(parse_url($result->you_tube_url, PHP_URL_QUERY), $yt_params);
                if (isset($yt_params['v'])) {
                    $this->session->set_userdata('yt_url', $yt_params['v']);
                }
            }
