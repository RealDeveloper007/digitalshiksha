...
class Login_model extends CI_Model
{
    ...
    public function login_check()
    {
     ...
     public function login_check_new()
    {
        $user = $this->input->get('user_email');
        $pass = md5($this->input->get('user_pass'));
        ...
        if ($result) 
        {
            if($result->user_role_id == 4 || $result->user_role_id == 5)
            {
                $role_name = $this->db->get_where('user_role', array('user_role_id' => $result->user_role_id))->row();
                $this->session->set_userdata('log', TRUE);
                $this->session->set_userdata('user_role', $role_name->user_role_name);
    ...
    public function login_check_in_api()
    {
        $user = $this->input->post('user_email');
        $pass = md5($this->input->post('user_pass'));
        ...
        if ($result) 
        {
            ...
            if($pass == $result->user_pass)
            {
                ...
                return ['status'=>false,'message'=>'password is not matched'];

            }
            } else {
            return ['status'=>false,'message'=>'Sorry! Your account is not activated '];
            }
        } else {
            return ['status'=>false,'message'=>'Sorry! record is not matched'];
        }
    }
    
     public function login_check_in_api_new_one()
    {
        $user = $this->input->post('user_email');
        $pass = md5($this->input->post('user_pass'));
        // $role = $this->input->post('user_type');
