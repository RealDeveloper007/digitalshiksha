...
class Blog_model extends CI_Model
{
    ...
    public function save()
    {
        ...
        $data = array();
        $data['blog_title'] = trim((string)$this->input->post('blog_title', TRUE));
        $data['blog_slug'] = trim((string)$this->input->post('blog_slug', TRUE));
        $data['blog_short_body'] = trim((string)$this->input->post('blog_short_body', TRUE));
        $data['blog_body'] = $this->input->post('post_descr');
        $selectedMediaType = $this->input->post('media_type', TRUE);

        if (!in_array($selectedMediaType, array('image', 'file', 'video'), TRUE)) {
            $this->session->set_flashdata('message', '<div class="alert alert-danger">Invalid media type selected.</div>');
            return FALSE;
        }

        if (!empty($_FILES['media']['name'])) {
            $config['upload_path'] = './uploads/blog_files/' . $selectedMediaType . '/';

            //echo $config['upload_path']; die;
            if ($selectedMediaType == 'image') {
                $config['allowed_types'] = 'gif|jpg|png|jpeg';
            } elseif ($selectedMediaType == 'file') {
            ...
            if (!$this->upload->do_upload('media')) {
                ...
                $file_name = $selectedMediaType . '/' . $upload_data['file_name'];
    ...
    public function update($id)
    {
        ...
        $selectedMediaType = $this->input->post('media_type', TRUE);
        $existingMediaType = $this->input->post('existing_media_type', TRUE);
        $existingAttachment = $this->input->post('existing_attachment');
        ...
        if ($_FILES['media']['name']) {
            ...
            // If no new file uploaded, preserve existing media unless a non-empty video URL is provided.
            if ($selectedMediaType === 'video') {
                $videoUrl = trim((string)$this->input->post('media'));
                if ($videoUrl !== '') {
                    $data['blog_media_type'] = 'video';
                    $data['blog_attachment'] = $videoUrl;
                } else {
                    $data['blog_media_type'] = $existingMediaType;
                    $data['blog_attachment'] = $existingAttachment;
                }
            } else if (!empty($selectedMediaType)) {
                // Image/file was selected but no file uploaded. Keep existing media.
                $data['blog_media_type'] = $existingMediaType;
                $data['blog_attachment'] = $existingAttachment;
            } else {
                // No media type posted, preserve existing media
                $data['blog_media_type'] = $existingMediaType;
                $data['blog_attachment'] = $existingAttachment;
            }
        }
